...
# Try to pull central path configuration; safe fallback if not present.
try:
...
# Resolve stop-words path with fallbacks: env override -> configured crisis-mmd stop_words -> local default.
stop_words_file = (
    os.environ.get("STOP_WORDS_FILE")
    or (str(Paths.CRISIS_MMD_ROOT / "stop_words" / "stop_words_english.txt") if Paths else None)
