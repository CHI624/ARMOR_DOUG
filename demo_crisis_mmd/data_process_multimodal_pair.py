...
# Locate stop-words via shared config when available.
try:
...
stop_words_file = (
    os.environ.get("STOP_WORDS_FILE")
    or (str(Paths.DEMO_STOP_WORDS / "stop_words_english.txt") if Paths else None)
    or "stop_words/stop_words_english.txt"
)
...
def read_dev_data_multimodal(data_file, text_file, label, tokenizer, MAX_SEQUENCE_LENGTH, delim, labels):
    """
    Prepare the data for both CrisisMMD and DMD.
    CrisisMMD uses ground truth labels, while DMD generates dummy labels.
    """
    ids = []
    data = []
    image_list = []
    ...
    if label is not None:
        ...
        # DMD pathway: create dummy labels
        print("DMD: Generating dummy labels.")
